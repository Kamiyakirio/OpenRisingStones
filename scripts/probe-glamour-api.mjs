...
const pages = await fetch("http://127.0.0.1:9333/json").then((response) =>
  response.json(),
);
const page = pages.find(
  (item) => item.type === "page" && item.url.includes("ff14risingstones"),
);
...
socket.send(
  JSON.stringify({ id: 1, method: "Page.navigate", params: { url: endpoint } }),
);
