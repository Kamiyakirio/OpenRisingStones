...
export async function fetchGlamours(options: {
  ...
  const items = records.map(toGlamour).filter((item): item is Glamour => Boolean(item))
...
function toGlamour(record: UnknownRecord, index: number): Glamour | null {
  ...
  const genderName = readString(record, ['gender_name', 'genderName', 'gender'])

  return {
    id: readNumber(record, ['id', 'glamour_id', 'glamourId', 'article_id']) ?? index,
    title: readString(record, ['title', 'name', 'glamour_name', 'glamourName']) ?? '未命名幻化',
    author: readString(record, ['nickname', 'user_name', 'username', 'author', 'character_name']) ?? '匿名冒险者',
    race: [raceName ?? raceIdMap[raceId ?? 0], genderName ?? genderIdMap[genderId ?? 0]]
      .filter(Boolean).join(' ') || '种族不限',
    job: readString(record, ['job_name', 'jobName', 'job', 'class_name', 'profession']) ?? '全职业',
    palette: readString(record, ['color_name', 'colorName', 'color', 'dye_name']) ?? '配色未标注',
    image,
    likes: readNumber(record, ['like_count', 'likeCount', 'likes', 'praise_count', 'like_num']) ?? 0,
    saved: readNumber(record, ['collect_count', 'collectCount', 'favorites', 'favorite_count']) ?? 0,
    featured: index === 0,
  }
}
...
function readImage(record: UnknownRecord) {
  const direct = readString(record, [
    ...
    'pic', 'picture', 'thumb', 'thumbnail', 'first_img', 'firstImg',
  ])
...
function findTotal(value: unknown, depth = 0): number | null {
  ...
  const direct = readNumber(value, ['total', 'total_count', 'totalCount', 'count'])
