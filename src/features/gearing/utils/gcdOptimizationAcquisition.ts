import rules from "../data/generated/rules.json";
/** Gearing domain module adapted from ffxiv-gearing (MIT); see licenses/ffxiv-gearing. */
export type GearAcquisitionKind =
  "tomestone" | "augmentedTomestone" | "raid" | "dungeon" | "other";

export interface GearAcquisitionPolicy {
  kind: GearAcquisitionKind;
  ringExclusivityGroup?: Exclude<GearAcquisitionKind, "other">;
  tomestoneCost: number;
  raidCost: number;
}

interface GearAcquisitionRule {
  kind: Exclude<GearAcquisitionKind, "other">;
  sourcePrefix: string;
  ringExclusive: boolean;
}

const acquisitionRules: readonly GearAcquisitionRule[] = rules.acquisition
  .acquisitionRules as readonly GearAcquisitionRule[];

const tomestoneCostsBySlot: Readonly<Record<number, number>> =
  rules.acquisition.tomestoneCostsBySlot;

const raidCostsBySlot: Readonly<Record<number, number>> =
  rules.acquisition.raidCostsBySlot;

export const progressionBudget = rules.acquisition.progressionBudget;

/** Unknown or renamed sources are deliberately treated as repeatable, zero-cost gear. */
export function getGearAcquisitionPolicy(
  source: string | undefined,
  slot: number,
  isWeapon: boolean,
  isChargedPointWeapon: boolean,
): GearAcquisitionPolicy {
  const rule = acquisitionRules.find((candidate) =>
    source?.startsWith(candidate.sourcePrefix),
  );
  if (rule === undefined)
    return { kind: "other", tomestoneCost: 0, raidCost: 0 };
  const absoluteSlot = Math.abs(slot);
  const tomestoneCost =
    rule.kind === "tomestone"
      ? isWeapon
        ? isChargedPointWeapon
          ? progressionBudget.pointWeaponCost
          : 0
        : (tomestoneCostsBySlot[absoluteSlot] ?? 0)
      : 0;
  const raidCost =
    rule.kind === "raid" ? (raidCostsBySlot[absoluteSlot] ?? 0) : 0;
  return {
    kind: rule.kind,
    ringExclusivityGroup: rule.ringExclusive ? rule.kind : undefined,
    tomestoneCost,
    raidCost,
  };
}
