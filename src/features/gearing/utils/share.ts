...
const reverseMapping = <T>(map: T[]) => {
  ...
  for (let index = 0; index < map.length; index++) {
    ret[String(map[index])] = index;
  }
  return ret as T extends number
    ? { [index: number]: number }
    : { [index: string]: number };
};
...
const jobEncode: {
  [index in G.Job]?: {
    index: number;
    statEncode: { [index in G.Stat]?: number };
  };
} = {};
...
export function stringify({
  ...
  while (!hasInvalidMateria && materiaEncode[minMateriaGrade] === undefined)
    minMateriaGrade++;
  ...
  for (const { id, materias, customStats } of gears) {
    ...
    if (id in specialGearEncode) {
      ...
      for (const materia of materias) {
        materiaCodes.push(
          materia === null || statEncode[materia[0]] === undefined
  ...
  if (gearCodes.length > 1) {
    ...
    if (
      gearIdDeltaRange >= gearCodes[0].id ||
      gearCodes[1].id - gearCodes[0].id <
        gearCodes[gearCodes.length - 1].id - gearCodes[gearCodes.length - 2].id
    ) {
    ...
    if (
      gearCodes[gearCodes.length - 1].id -
        gearCodes[gearCodes.length - 2].id ===
  ...
  for (let i = gearCodes.length - 1; i >= 0; i--) {
    ...
    write(
      gearTypeEncode[
        customStats === undefined ? materias.length : GearType.Customizable
      ],
      gearTypeRange,
    );
  }
  ...
  if (
    syncLevel !== undefined ||
    jobLevel !== jobLevelDecode[ranges.jobLevel - 1]
  ) {
...
export function parse(s: string): G.Gearset | "shb" | "ew" {
  ...
  const read = (range: number): number => {
    if (!Number.isSafeInteger(range) || range <= 0)
      throw new Error("Invalid share encoding range.");
  ...
  if (version !== 5) throw new Error("Unsupported share version.");
  ranges.useVersion(version);
  ...
  const jobLevel =
    jobLevelDecode[synced ? read(ranges.jobLevel) : ranges.jobLevel - 1];
  ...
  let id = -1;
  while (input !== 0n) {
    if (gears.length >= 20) throw new Error("Too many shared gear items.");
    ...
    if (id === -1) {
      ...
      id +=
        (read(gearIdDeltaRange) - (input === 0n ? 1 : 0)) *
        gearIdDeltaDirection;
    }
