...
export const ProductionMateriaCalculationPanel = mobxReact.observer(() => {
      ...
      stats.map((stat) => [
        ...
        String(
          store.equippedStatsWithoutFood[stat] ??
            store.productionMateriaBaseStats[stat] ??
            0,
        ),
      ]),
    ),
  ...
  const [result, setResult] =
    React.useState<ProductionMateriaOptimizationResult>();
  const [calculating, setCalculating] = React.useState(false);
  const requestId = React.useRef(0);
  const parsed = Object.fromEntries(
    stats.map((stat) => [stat, Number(targets[stat])]),
  ) as G.Stats;
  ...
  const tooHighStats = new Set(
    stats.filter(
      (stat) =>
        Number.isInteger(parsed[stat]) &&
        parsed[stat]! > (store.productionMateriaMaximumStats[stat] ?? 0),
    ),
  );
  React.useEffect(
    () => () => {
    ...
    <div className="production-materia-optimization card">
      ...
      <div className="production-materia-optimization_targets">
        {stats.map((stat) => {
          ...
          return (
            <label
              className="production-materia-optimization_target"
              key={stat}
            >
              ...
              <small className={classNames(invalid && "-error")}>
                {tooHighStats.has(stat)
                  ? `最高 ${store.productionMateriaMaximumStats[stat]}`
                  : `基础 ${minimum}`}
              </small>
            </label>
          );
        ...
        {invalidStats.size > 0 && (
          <div className="production-materia-optimization_validation">
            目标属性不能低于装备基础属性，且必须为整数。
          </div>
        )}
        ...
        <Button
          disabled={
            invalidStats.size > 0 || tooHighStats.size > 0 || calculating
          }
          onClick={calculate}
        >
          {calculating ? "计算中" : "计算"}
...
const ProductionMateriaResultView = mobxReact.observer<{
  ...
  if (result === undefined) {
    return (
      <div className="production-materia-optimization_empty">
        属性上限以内的溢出数值不会计入结果。
      </div>
    );
  }
  if (result.status === "error") {
    return (
      <div className="production-materia-optimization_message -error">
        {result.message}
      </div>
    );
  }
      ...
      <div className="production-materia-optimization_message -warning">
        ...
        <div>
          {stats
            .map(
              (stat) =>
                `${G.statNames[stat]}单项最高 ${result.maximumStats[stat]}`,
            )
            .join("，")}
        </div>
      </div>
  ...
  const materiaCount = result.plan.reduce(
    (sum, gear) =>
      sum +
      gear.materias.filter((materia) => materia.stat !== undefined).length,
    0,
  );
  return (
      ...
      <table className="production-materia-optimization_plan table">
        <tbody>
          {result.plan.flatMap((gearPlan) => {
            const materias = gearPlan.materias.filter(
              (materia) => materia.stat !== undefined,
            );
            if (materias.length === 0) return [];
            const slot = store.schema.slots.find(
              (item) => item.slot === gearPlan.slot,
            );
            return [
