...
export const MateriaClearPanel = mobxReact.observer(() => {
  ...
  const slotItems = store.schema.slots.flatMap((slot) => {
    const gear = store.equippedGears.get(slot.slot.toString()) as
      IGearUnion | undefined;
    if (gear === undefined || gear.isFood) return [];
    const materiaCount = gear.materias.filter(
    ...
    slot: G.SlotSchema;
    gear: Exclude<IGearUnion, { isFood: true }>;
    materiaCount: number;
  }[];
  ...
  const selectedMateriaCount =
    mode === "all"
      ...
      : slotItems.reduce(
          (sum, item) =>
            sum + (selectedSlotSet.has(item.slot.slot) ? item.materiaCount : 0),
          0,
        );
  return (
    <div className="materia-clear">
      ...
      {slotItems.length === 0 ? (
        <div className="materia-clear_empty">
          当前已选装备没有已配置的魔晶石。
        </div>
      ) : (
        <>
              ...
              {slotItems.map(({ slot, gear, materiaCount }) => (
                <button
                  ...
                  className={classNames(
                    "materia-clear_slot",
                    selectedSlotSet.has(slot.slot) && "-selected",
                  )}
                  onClick={() => {
                    setSelectedSlots((slots) =>
                      slots.includes(slot.slot)
                        ? slots.filter((s) => s !== slot.slot)
                        : slots.concat(slot.slot),
                    );
                  }}
                >
                  <span className="materia-clear_slot-name">
