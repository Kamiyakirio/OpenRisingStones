...
function isValidGcdTarget(gcd: number): boolean {
  return (
    Number.isFinite(gcd) &&
    gcd >= gcdOptimizationMinTargetGcd &&
    gcd <= gcdOptimizationMaxTargetGcd
  );
}
...
export const MateriaGcdCalculationPanel = mobxReact.observer(() => {
  ...
  const [targetGcd, setTargetGcd] = React.useState(() =>
    formatGcdTarget(store.equippedEffects?.gcd),
  );
  ...
  const baseSpeed =
    store.baseStats[speedStat] ?? G.jobLevelModifiers[store.jobLevel].sub;
  const targetRequiredSpeed = targetGcdValid
    ...
    : Infinity;
  const speedRangeBelowBase =
    speedRangeValuesValid && maximumSpeedNumber < baseSpeed;
  const speedRangeConflictsWithTarget =
    speedRangeValuesValid &&
    !speedRangeBelowBase &&
    targetRequiredSpeed > maximumSpeedNumber;
  const speedRangeValid =
    speedRangeValuesValid &&
    !speedRangeBelowBase &&
    !speedRangeConflictsWithTarget;
  const speedRange: GcdOptimizationSpeedRange | undefined =
  ...
  return (
    <div className="materia-gcd-optimization card">
        ...
        <div className="materia-gcd-optimization_target">
          ...
          <Button
            ...
            disabled={
              ...
              calculating ||
              (mode === "all" &&
                (store.gcdOptimizationSelectedGearIds.length === 0 ||
          ...
          {speedRangeEnabled && (
            <div className="materia-gcd-optimization_speed-range">
              <div className="materia-gcd-optimization_speed-range-controls">
                <span className="materia-gcd-optimization_label">
                  最终{speedStatName}
                </span>
                <TextField
        ...
        <>
          <div className="materia-gcd-optimization_gear-selector">
            ...
            <span className="materia-gcd-optimization_gear-selector-actions">
              <Button
                disabled={
                  store.gcdOptimizationSelectedGearIds.length ===
                  candidateGearIds.length
                }
                onClick={() => {
          ...
          <div className="materia-gcd-optimization_source-selector">
            <span className="materia-gcd-optimization_source-selector-label">
              按获取途径
            </span>
            <span className="materia-gcd-optimization_sources">
              {candidateGearSourceGroups.map(([source, gearIds]) => {
                ...
                return (
                  <label
                    className="materia-gcd-optimization_source"
                    key={source}
                  >
                    <input
                      type="checkbox"
                      ...
                      onChange={() => {
                        const nextSelectedGearIds = new Set(
                          store.gcdOptimizationSelectedGearIds,
                        );
                        ...
                        store.setGcdOptimizationSelectedGearIds(
                          candidateGearIds.filter((gearId) =>
                            nextSelectedGearIds.has(gearId),
                          ),
                        );
                      }}
                    />
          ...
          <div className="materia-gcd-optimization_progression">
            ...
            {progressionEnabled && (
              <div className="materia-gcd-optimization_progression-tip">
                <div>
                  点数武器默认不参与计算。如需使用，请取消勾选其他武器，仅留点数武器一个选项；
                  将消耗 500 点数。
                </div>
                <div>
            ...
            {progressionEnabled && !progressionWeeksValid && (
              <div className="materia-gcd-optimization_progression-validation">
                准备周数必须是 0–10 之间的整数。
              </div>
            )}
          </div>
      ...
      {calculating && <Button onClick={resetResult}>取消计算</Button>}
...
const MateriaGcdOptimizationResultView = mobxReact.observer<{
  ...
  if (result.status === "error") {
    return (
      <div className="materia-gcd-optimization_message -error">
        {result.message}
      </div>
    );
  }
    ...
    if (result.speedRange !== undefined) {
      return (
        <div className="materia-gcd-optimization_message -warning">
          ...
          {result.closestSpeed !== undefined &&
            result.closestGcd !== undefined && (
  ...
  return (
    <div className="materia-gcd-optimization_result">
      <table className="materia-gcd-optimization_summary table">
        <tbody>
          ...
          <tr>
            ...
            <td
              className={classNames(
                result.damageDelta >= 0 ? "-positive" : "-negative",
              )}
            >
      ...
      {customStatPlans.map((plan) => (
        <div
          className="materia-gcd-optimization_note"
          key={plan}
        >{`自动配置 ${plan}`}</div>
      ))}
      {result.customSkipped && (
        <div className="materia-gcd-optimization_note">
          自动选择装备时已跳过未配置自定义属性的装备。
        </div>
      )}
      <Button
