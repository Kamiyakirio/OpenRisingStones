  ...
  return (
    <table className="materias table card">
      <tbody>
            ...
            stat in G.materias && (
              <tr
                ...
                className={classNames(
                  "materias_row",
                  materia.gear.currentMeldableStats[stat]! <= 0 && "-invalid",
                )}
              >
                <td
                  className={classNames(
                    "materias_meldable",
                    materia.gear.currentMeldableStats[stat]! < 0 &&
                      "-overflowed",
                  )}
                >
                  {materia.gear.currentMeldableStats[stat]}
                </td>
                <td className="materias_stat-name">
                  {G.materiaStatNames[stat].slice(0, 2)}
                </td>
                  ...
                  <td
                    ...
                    onClick={() => {
                      if (materia.stat === undefined) {
                        // Only for initial melding, not for changing materia
        ...
        <tr>
          <td className="materias_meldable-title" colSpan={2}>
            可镶嵌值
          </td>
          <td
            className={classNames(
              "materias_remove",
              materia.stat === undefined && "-selected",
            )}
            colSpan={
              materia.meldableGrades.length - (materia.canRestricted ? 1 : 0)
            }
