...
export const Gear = mst.types
  ...
  .views((self) => ({
    ...
    get color(): GearColor {
      ...
      return (
        (gearColorScheme === "source" && sourceColors[source.slice(0, 2)]) ||
        rarityColors[rarity] ||
        "white"
      );
    },
    get syncedLevel(): number | undefined {
      ...
      if (syncLevel >= this.level && jobLevel >= this.equipLevel)
        return undefined;
      const jobLevelSyncedLevel = Math.min(
        this.level,
        G.syncLevelOfJobLevels[jobLevel],
      );
    ...
    get currentMeldableStats(): G.Stats {
      ...
      for (const stat of Object.keys(this.caps) as G.Stat[]) {
        stats[stat] =
          this.totalMeldableStats[stat]! - (this.materiaStats[stat] ?? 0);
      }
      return stats;
    },
    get statHighlights(): { [index in G.Stat]?: boolean } {
      ...
      for (const stat of Object.keys(this.bareStats) as G.Stat[]) {
        ret[stat] =
          G.statHighlight[stat] &&
          (this.bareStats[stat] ?? 0) >= (this.caps[stat] ?? Infinity);
      }
      return ret;
    },
  ...
  .actions((self) => ({
    setCustomStat(stat: G.Stat, value: number) {
      if (value > 0) {
        self.customStats!.set(
          stat,
          value <= G.customStatMax ? value : G.customStatMax,
        );
      } else {
    ...
    initialize() {
      ...
      if (self.materias.length < materiaSlot) {
        self.materias.push(
          ...new Array(materiaSlot - self.materias.length).fill({}),
        );
      }
...
// Presentation metadata may briefly lag during development data replacement.
const rarityColors = (rules.colors?.rarity ?? {}) as Record<number, GearColor>;
const sourceColors = (rules.colors?.source ?? {}) as Record<string, GearColor>;
