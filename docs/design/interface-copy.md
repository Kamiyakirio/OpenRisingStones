...
Status: copy specification accompanying the finalized DESIGN.md for direction B.
The current application has not yet been updated to use this copy.
...
| Location                  | Current copy                                      | Proposed copy or treatment        |
| ------------------------- | ------------------------------------------------- | --------------------------------- |
...
The existing feature buttons can keep their whole-row click target. An arrow may
...
These replacements are intended for the corresponding redesign, not blind global
substitution. Preserve the actual event handler and operation behind each label.
...
| Glamour time claim           | 今日幻化推荐                                                                   | Remove unless the displayed content is verified as today's selection |
| Glamour source               | 灵感来源 / 冒险者真实投稿                                                      | 来源 / 石之家                                                        |
...
| Login expiry                 | 石之家会话已经失效。重新登录后会自动恢复幻化推荐与装备搜索。                   | 登录已过期，请重新登录石之家。                                       |
...
| Glamour login required          | 登录石之家后可浏览幻化投稿和装备详情。               | 登录石之家                                         |
...
| Gearing import failed           | 导入失败，请检查分享码和装备数据。当前配装未被覆盖。 | Preserve the entered code for correction           |
...
wrapping, zoom, keyboard names, and both themes when this copy is implemented.

## Source references

- `src/pages/HomePage.tsx`: feature entrypoints and current home copy.
- `src/features/glamour/components/GlamourHero.tsx`: glamour introduction and source.
- `src/features/glamour/components/GlamourLoginWall.tsx`: login boundary.
